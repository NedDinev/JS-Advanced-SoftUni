...
describe("value prop", () => {
  ...
  it("returns true if arg value is 0", () => {
    expect(new PaymentPackage("valid", 0).value).to.equal(0);
  });
});
...
describe("VAT prop", () => {
  ...
  it("throws an error if value is not an integer", () => {
    expect(() => (new PaymentPackage("valid", "gosho").VAT = "gosho")).to.throw(
      Error
    );
  });
  ...
  it("throws an error if value is a string", () => {
    expect(() => (new PaymentPackage("valid", "100").VAT = "100")).to.throw(
      Error
    );
  });
});
